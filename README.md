...
把一张 PNG 一次性做成**按钮式轻按回弹 GIF**：轻微按下 → 轻柔回弹（略过冲）→ 静止，动画**只循环播放一遍**（播完停在原始画面），居中缩放、保留透明背景。
...
| `--amplitude` | 1.0 | 按压幅度系数，1.3 ≈ 按得更深，0.6 ≈ 更轻 |

## 动画说明

* 12 帧按钮式轻按回弹（精确 0.5 秒）：整体缩小按下 → 轻微过冲回弹 → 静止，居中缩放。
